在原本的结构里面，由于默认服务引用的都是ABP原生的模块，所以结构目录里面没有包含modules目录，这里我们添加一个modules目录，用于存放我们的自定义模块。<br />在shared里面，我们再抽一个EventData的模块，用于消息队列共用数据实体。修改后结构如下图所示：<br />![](https://cdn.nlark.com/yuque/0/2023/jpeg/730088/1672887325508-cea59ba6-0495-45c2-8143-8822c88b12d9.jpeg)
<a name="T9qs3"></a>
## 开始搭建
由于我们没有商业版的代码生成器，那就纯手工搭建这个结构了。这里我们使用VS Code作为编辑器配合dotnet cli操作<br />创建新的空白解决方案，后续通过再VS来编辑解决方案的内容。
```powershell
dotnet new sln -n FunShow
```
然后在解决方案目录下创建目录<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/730088/1672899249508-d91edf6a-7b2a-4c00-a135-c9b408780990.png#averageHue=%23fcfbf8&clientId=u6849452c-0046-4&crop=0&crop=0&crop=1&crop=1&from=paste&height=162&id=u1be4713b&margin=%5Bobject%20Object%5D&name=image.png&originHeight=162&originWidth=259&originalType=binary&ratio=1&rotation=0&showTitle=false&size=3989&status=done&style=none&taskId=uee144f4d-7043-4931-a36b-459524a86b9&title=&width=259)
<a name="xEKFy"></a>
## 创建Shared项目
...
```
FunShow.Shared.Hosting.Gateways<br />这里网关我们使用yarp
```xml
<Project Sdk="Microsoft.NET.Sdk">
...
```
这个类用于扩展IHostBuilder方法，配置网关读取配置文件，这里采用yarp作为网关组件，原商业版微服务模板采用的是ocelot。<br />添加类YarpSwaggerUIBuilderExtensions.cs，用于配置swagger
```csharp
using System.Linq;
